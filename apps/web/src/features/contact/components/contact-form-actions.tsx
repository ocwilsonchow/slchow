...
export function ContactFormActions({
  ...
  return (
    <div className="flex items-center justify-end gap-4">
      {isBusy ? null : (
        <span className="mr-auto inline-flex items-center gap-1 text-sm text-content-subdued">
      ...
      <div className="flex items-center gap-2">
        {canGoBack ? (
          <motion.button
            type="button"
            className="text-content-subdued transition-colors hover:text-content-ink font-semibold border px-4 py-2 rounded-xl text-sm hover:bg-surface-alpha/25"
            onClick={onBack}
            disabled={isBusy}
            aria-label={t("back")}
            whileTap={{ scale: 0.9 }}
        ...
        <motion.button
          ...
          type="submit"
          className="inline-flex items-center gap-2 text-content-ink disabled:text-content-subdued font-semibold border px-4 py-2 rounded-xl text-sm bg-surface-alpha/50 hover:bg-surface-alpha"
          disabled={isBusy}
          aria-label={
