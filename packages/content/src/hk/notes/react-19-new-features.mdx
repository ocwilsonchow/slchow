...
React 19.2 在這個基礎上加入了 `Activity`、`useEffectEvent`，以及更好的效能工具。以下是最實用的功能，以及決定何時使用時值得記住的注意事項。
...
Actions 最顯眼的用途是 `<form>` 上新的 `action` prop。不用再攔截 `onSubmit`、讀取表單、追蹤 loading flag，再手動重置表單——可以直接把函式傳給它。
...
當 server 結果應該變成 UI state 時，適合用 `useActionState`；若 controls 只需要知道周圍 form 是否 pending，則用 `useFormStatus`。
...
Optimistic updates 最適合預期結果很明顯、失敗可以解釋或還原的情況。destructive operations 值得更謹慎，假裝成功會誤導用戶的情況也一樣。
...
`use` 是 render 與可能尚未就緒的 resource 之間的橋樑，而不是取代每個 data-fetching library。
...
Frameworks 已經處理了其中不少工作，所以在直接呼叫這些 API 之前，先查看 framework 的慣例。
...
這提供比 React DevTools Profiler 更底層的視角。當緩慢的互動牽涉 scheduling、瀏覽器工作與 React rendering，而不是某個明顯昂貴的 component 時，它很合適。
...
React、React DOM，以及它們的 TypeScript definitions 應一起升級，使用 framework 支援的最新 patched React 19 release。
...
**升級與功能採用應分成兩個步驟**。先讓既有應用在 React 19 上運作，再在 Actions、`use` 或 `Activity` 真正簡化某個 pattern 的地方引入它們。Major upgrade 若沒有與大規模 rewrite 混在一起，會更容易 debug。
...
React 19 的主題是 coordination。Actions 協調 async mutations 與 pending 及 optimistic UI。`use` 協調 rendering 與 resources。Document APIs 協調 metadata、styles 與 scripts。`Activity` 協調可見與背景工作，而 `useEffectEvent` 把 synchronization 與 event-like behavior 分開。

最常保留的功能是 form Actions、`useActionState`、`useOptimistic`、ref as a prop，以及 `useEffectEvent`。`use`、`Activity` 與 server rendering APIs 更依賴 framework 支援與應用架構，但它們指向一種 loading、mutations 與 rendering 被設計成一起運作的 React model。
