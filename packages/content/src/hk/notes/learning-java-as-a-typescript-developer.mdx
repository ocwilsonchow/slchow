...
Hono 是你用 request 呼叫的 function。Spring Boot 是一個 **建構 beans 的 process**，再把 HTTP map 到 methods。那個分裂的 NestJS 表親 —— TypeScript container，外圍 Express 或 Fastify —— 見 [深入理解 NestJS](/notes/understanding-nestjs-in-depth)。
