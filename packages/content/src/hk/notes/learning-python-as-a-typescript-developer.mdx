...
這篇 note 是以 **Hono** developer 身分讀 **FastAPI** API 時用的 mapping。Samples 對準 **Python 3.12+**：type parameter syntax、`match`、dataclasses。**Pydantic v2** 與 **SQLAlchemy 2** 各出現一次，作為 Zod 與 Drizzle 的表親。不是把這個 site 重寫成 Python。Typed Hono stack 見 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/backend-apis-hono-sst)。Types 真的活過 compilation 的 Java 兄弟篇是 [以 TypeScript Developer 身分學 Java](/notes/learning-java-as-a-typescript-developer)。你已有的 object model 見 [TypeScript Class 與 Runtime Identity](/notes/typescript-classes-and-runtime-identity)。
