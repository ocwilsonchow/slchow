...
description: 一套 FastAPI app，跑在 Lambda 或 Fargate——Pydantic、OIDC JWT、SQLAlchemy、Alembic、OpenAPI、Terraform、RDS Proxy、WAF 與 alarms
date: 2026-09-01
category: backend
---
...
Python 作為語言 —— duck typing、GIL、`Depends` 對比 Hono —— 見 [以 TypeScript Developer 身分學 Python](/notes/learning-python-as-a-typescript-developer)。SST 上的 TypeScript 兄弟篇是 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/_backend-apis-hono-sst)。本站自己的 deploy loop 仍是 SST；這裡的 Terraform 是獨立的 architecture example，disclaimer 與 [打造 Event-Driven 票務 Backend](/notes/building-an-event-driven-ticketing-backend) 相同。Tokens 見 [OAuth 2.0 與 OIDC 解釋](/notes/oauth-2-and-oidc-explained)。這篇筆記涵蓋 typed FastAPI API，一路到 production 的 protect、monitor 與 recover。
...
Contract 是一條 route 接受什麼、回傳什麼，以及 errors 長什麼樣。一份 **Pydantic** model 同時驅動 **validation**、**OpenAPI** 與 response body。普通函數上的 type annotation 只是註釋 —— [以 TypeScript Developer 身分學 Python](/notes/learning-python-as-a-typescript-developer)。REST 形態見 [System Design 裡的 API Design](/notes/api-design-in-system-design)。
...
- 共享的 `ErrorBody` 讓 `400` / `404` / `409` 保持一致。API Gateway JWT 失敗用更小的 gateway 形狀表示 `401` / `403` —— 兩邊都要寫進文件。
...
PostgreSQL 是 durable authority。SQLAlchemy 2 讓 schema 與 queries 保持 typed，同時不把 SQL 藏起來。Statement 模型見 [SQL 核心概念](/notes/core-sql-concepts)。在 concurrency 下，優先用 **database constraints**（unique email、`timestamptz`），而不是 check-then-insert。
...
這個 API 是 **resource server**。它不跑 login form。你不擁有的 issuer（Auth0、Okta、Cognito，或任何 OIDC provider）簽發 access token。Issuer URL 與 audience 是 Terraform variables。Roles、grants，以及 ID token 與 access token 的區分，見 [OAuth 2.0 與 OIDC 解釋](/notes/oauth-2-and-oidc-explained)。
...
- Authentication 回答 *誰*。Authorization（roles、org membership、RLS）留在 app 裡 —— 見 [multi-tenant 後端筆記](/notes/multi-tenant-backend-drizzle-hono-better-auth)。
...
- 讓 `/api/health` 與 `OPTIONS` **離開** authorizer。
...
- FastAPI 從同一個 `app` object 提供 `/openapi.json` 與 `/docs`。你已經點名了 `response_model` 與 `responses`。
...
- 在 production 對 `/docs` 與 `/openapi.json` 做 stage-gate 或 authenticate。公開的 reference 就是免費偵察。
...
一份 image。兩份 compute resources。共享 VPC、RDS、Proxy 與 OIDC variables。沒有 MSK、沒有 global Aurora、沒有 purchase cells —— 那些屬於 [票務筆記](/notes/building-an-event-driven-ticketing-backend)。
...
- 明確的 public `OPTIONS` 與 `/api/health` 很重要：帶 JWT authorizer 的 `$default` 會拒絕 browser preflight 與 liveness。
...
- 用 `aws_wafv2_web_acl_association` 把 **WAFv2** 掛到 HTTP API 的 stage ARN，方式與掛在 ALB 上相同。
...
- Restore drill：snapshot → 非 prod 的 `DATABASE_URL` → `alembic upgrade head` → `/api/health` + 一條關鍵路徑 → 記錄真實 RTO。

**Failure:** 把 Multi-AZ 叫作「disaster recovery」。那是同一 region 內的 high availability，不是 cross-region DR。策略見 [System Design 裡的 Disaster Recovery](/notes/disaster-recovery-in-system-design)。
...
- 可接受的 trade-offs：container-image 的 **cold start**（比 zip 大）、Proxy 多一跳、CPython 的 GIL 使 CPU-bound 工作不屬於這條 request path（[學 Python](/notes/learning-python-as-a-typescript-developer)），以及你必須保護的 Terraform state —— 本站的 app stacks 仍在 [SST](/notes/using-sst-for-aws-infra-and-devops)。每一層都保持可替換。
...
這條脊柱的 TypeScript 版本是 [Hono 筆記](/notes/_backend-apis-hono-sst)。票務規模的 Fargate 是 [event-driven 票務筆記](/notes/building-an-event-driven-ticketing-backend)。
...
<Accordion type="single" collapsible>
  <AccordionItem value="fastapi-arch">
    ...
    <AccordionContent>
      ...
      - 預設形態：一個 **FastAPI Lambda** 放在 **API Gateway HTTP API** 後面，使用內建 **JWT authorizer**。被拒絕的 tokens 永遠不會 invoke function。
      - 同一個 app 在 Fargate 上用 `uvicorn` 跑。唯一的 AWS-specific module 是 `app/lambda_handler.py`（`Mangum(app)`）。Business logic 不要 import Mangum。
      - 選 **Lambda**：流量尖峰或閒置、handlers 短、authorizer 先拒絕。
      - 選 **Fargate**：穩定流量、長請求、streaming、workers，以及真正的 connection pool。
      - 在 Lambda 上，identity 是 infrastructure 邊界。在 Fargate 上，它在 request 打到 task 之後於 process 內執行。兩邊都仍需要 edge controls。兩邊都經 RDS Proxy 到達 Postgres。

    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="fastapi-pydantic">
    <AccordionTrigger>2. 一份 Pydantic model 驅動什麼，為什麼那就是 contract？</AccordionTrigger>
    <AccordionContent>

      - Contract 是一條 route 接受什麼、回傳什麼，以及 errors 長什麼樣。一份 Pydantic model 同時驅動 **validation**、**OpenAPI** 與 JSON body。
      - `response_model=UserRead` 是 HTTP 邊界。在 service 裡把 ORM → Pydantic。沒有 Pydantic 的 function annotation 只是註釋。
      - 共享的 `ErrorBody` 讓 `400` / `404` / `409` 保持一致。即使 FastAPI 在 Lambda 路徑上不發出 gateway `401` / `403`，也要寫進文件。
      - `/openapi.json` 是可攜式的 artifact。`/docs` 是 presentation —— production 裡做 stage-gate。
      - Production 預設是一個 app object，讓 middleware、errors、OpenAPI 與 DB setup 留在一處。

    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="fastapi-proxy">
    <AccordionTrigger>3. SQLAlchemy 在 Lambda 上該怎麼放，migrations 何時跑？</AccordionTrigger>
    <AccordionContent>

      - PostgreSQL 是 durable authority。在 concurrency 下，優先用 **database constraints**，而不是 check-then-insert。
      - 把 engine 建立在 handler **之外**。Lambda 上用 `NullPool` 與 `statement_cache_size=0`，避免 RDS Proxy 把 session pin 到每個 warm environment。Fargate 上用小的 `QueuePool`。
      - 限制 reserved concurrency。Proxy 是 multiplexor，不是每次 invoke 開五條 connections 的許可證。
      - Migrations 是 **deploy step**，絕不是 request path 的一部分。先在可丟棄的 DB 上 `alembic upgrade head`，再在新 image 接收流量之前 apply。
      - Secrets 來自 **Secrets Manager**，不是 committed 的 env file。

    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="fastapi-jwt">
    <AccordionTrigger>4. OIDC access token 在哪裡檢查，什麼應離開 authorizer？</AccordionTrigger>
    <AccordionContent>

      - 這個 API 是 **resource server**。Terraform 接收 `oidc_issuer` 與 `oidc_audience`。這篇筆記沒有 Cognito resource。
      - 在 Lambda 上，HTTP API **JWT authorizer** 在 Mangum 跑之前驗證 `iss`、`aud` 與 JWKS signature。在 Fargate 上，`require_user` 用 `PyJWKClient` 做同樣的事。
      - 讓 `/api/health` 與 `OPTIONS` **離開** authorizer。Identity 是 `sub`，絕不是 `X-User-Id`。
      - Authorization（roles、org、RLS）留在 app 裡。ID token 不是 API credential。
      - **Failure:** 帶 credentials 的 wildcard CORS origin，或跳過 authorizer、為每一枚無效 bearer token 付錢去 invoke。

    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="fastapi-terraform">
    <AccordionTrigger>5. Terraform 如何接上兩種 runtime，什麼不在範圍內？</AccordionTrigger>
    <AccordionContent>

      - 一份 container image。Lambda 經 `awslambdaric` 把 `command` 改成 `app.lambda_handler.handler`。Fargate 保留 Dockerfile 的 `CMD`（`uvicorn`）。
      - HTTP API 的 `$default` 走 JWT authorizer。ECS service 坐在帶 WAFv2 的 ALB 後面。RDS + RDS Proxy 共享。
      - 這是精簡 example：沒有 MSK、沒有 Aurora Global、沒有 purchase cells。票務規模是另一篇。本站自己的 stacks 仍在 SST。
      - 對 Lambda `Errors` 與 ALB `5xx` 告警。Durable state 是 Postgres。Compute 是 image tag。
  ...
  <AccordionItem value="fastapi-pytest">
    <AccordionTrigger>6. 如何用 pytest 對 FastAPI app 做單元測試？</AccordionTrigger>
    <AccordionContent>

      - 測試寫在 Mangum **之下**，用 `TestClient` 打同一個 `app`。命令是 `uv run pytest`。不需要 AWS event、container，或真實 issuer。
      - 經 `dependency_overrides` stub `require_user`。那是 seam。write path 不能碰 Postgres 時，再 override `get_session`。
      - 斷言 contract：`/api/health` 是 public，壞 body 是 `400` + `VALIDATION_ERROR`，缺失 bearer 是 **403**（`HTTPBearer`）。
      - ORM → Pydantic 的 mapping 放在 service function，不經 HTTP 也能測。`select`、unique violations 與 Alembic 用可丟棄的 Postgres —— 仍是 pytest，不是 route unit test。
      - **Failure:** 在每個 test 裡拼 API Gateway event 再呼叫 `handler`。那測的是 Mangum，不是 API。
