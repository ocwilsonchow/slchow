...
PostgreSQL 是持久化的權威來源。Drizzle 讓 schema 與 queries 保持 typed，又不隱藏 SQL。Statement 模型見 [SQL 核心概念](/notes/core-sql-concepts)。在 concurrency 下，優先用 database constraints，而不是 check-then-insert。
