...
當我建立 backend API 時，希望從 request 到 database 的路徑很短，也希望 **validation**、**documentation** 與 **infrastructure** 都來自同一份程式碼。這套 stack 是 **Hono**、**Better Auth**、**Drizzle**、**PostgreSQL**、**Zod OpenAPI**、**Scalar**，以及跑在 **AWS Lambda** 或 **ECS Fargate** 上的 **SST**。
