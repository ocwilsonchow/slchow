...
Typed API 一路到 production 的 protect、monitor 與 recover 見 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/_backend-apis-hono-sst)。Stages 與 deploy loop 見 [用 SST 管理 AWS 基礎設施與 DevOps](/notes/using-sst-for-aws-infra-and-devops)。具體的 outbox、inbox 與 compensation 流程見 [打造 Event-Driven 票務 Backend](/notes/building-an-event-driven-ticketing-backend)。這篇 note 講的是 queue。
...
| [**Kafka / Kinesis**](/notes/kafka-in-system-design) | Replayable log | 多個獨立 readers、有序 history、replay。 |
