---
title: 2026 個人發展計劃
description: 從一位扎實的 frontend 與 applied AI engineer，成長為具備更深技術判斷、reliability practices 與團隊影響力的 senior engineer 的計劃
date: 2026-08-06
category: personal
---

履歷對照 **Senior Frontend Engineer** 與 **Applied AI Engineer** 的期望進行了審視。結果令人鼓舞，但也暴露出一個有用的差距：履歷證明一位 senior frontend 與 applied AI engineer 能建構並交付複雜產品，卻未能一貫展現 senior engineer 所期望的更廣泛影響力、**quality systems**，以及 **operational judgment**。

這是對履歷證據的評估，而不是對能力的完整量度。有些弱點可能是尚未清楚記錄的經驗；另一些則是仍需刻意培養的能力。

<br />

此計劃 2026 年的目標不是收集更多 framework 名稱，而是更擅長做出 **technical decisions**、降低風險、提升其他工程師的產出，並在 production 中可靠地運營 frontend 與 AI systems。
...
此 profile 最強的定位，落在 **frontend software engineering** 與 **applied AI systems** 的交匯處。
...
Profile 反映已交付超過 15 個 **React** 與 **React Native** 應用，建構共用 **component libraries**，在 **Figma** 設計介面，優化 web 與 mobile **performance**，開發 typed APIs，並在 **AWS** 上部署基礎設施。也反映 **agent workflows**、**durable streaming**、**memory**、**RAG** pipelines、retrieval 與 reranking、**observability**，以及 **evaluation suites**。
...
這些數字不是客觀技能評分。它們用來辨識：經驗中哪些部分對 hiring team 已經有說服力，哪些部分仍依賴假設。
...
| --- | --- | --- |
| **Frontend delivery and performance** | 15+ React and React Native apps；在 load time、search latency、Core Web Vitals、deployment speed 與 crash-free sessions 上有可量度的提升 | 證明能交付並改善面向用戶的系統 |
| **End-to-end ownership** | Design、frontend、typed APIs、data、AWS infrastructure、deployment 與 observability | 支持跨越產品與系統邊界做決策 |
...
| **Product collaboration** | 直接與 client 與 stakeholder 做 scoping、UX iteration 與 delivery | 證明能把模糊需求轉成技術工作 |

<br />

### Frontend Delivery and Performance

Profile 涵蓋把 frontend product 從設計帶到 production。經驗涵蓋 **React**、**Next.js**、**React Native**、**design systems**、animation、data visualization、forms、state 與 server data、**CMS** integrations、search，以及 deployment。
...
這些結果比技術清單更有力，因為它們證明能診斷問題，並改善用戶或工程師能感受到的結果。

<br />

### End-to-End Ownership

工作習慣延伸到 component 邊界之外。Profile 包括在 frontend 應用之外建構的 **typed APIs**、**authentication**、database access、search、**cloud infrastructure**、**observability**，以及 **deployment pipelines**。

這種廣度支持更好的 frontend 決策。它反映對資料來源、API 保證、authentication 對 navigation 與 caching 的影響、deployment architecture 對 performance 的影響，以及 production failures 可能出現位置的理解。

<br />

### Applied AI Systems

Profile 上的 AI 經驗不只是在應用裡加一個 chat completion endpoint。它包括：
...
Profile 包括直接與 clients 與 stakeholders 合作，做 scoping、迭代 UX，並交付 releases。Seniority 要求的不只是正確實作規格，還要找出真正的 **constraint**、解釋 **trade-offs**，並幫助團隊決定應該建構什麼。

已有部分這類證據，但決策及其影響仍需更清楚地記錄。
...
當前 profile 最大的弱點不是實作能力，而是缺少證據證明工作提升了團隊產出。

履歷說曾帶領 projects、review 過 frontend code，並建立 shared libraries。它沒有說明：

- 支援過多少工程師
- 是否 **mentored** 其他 developers
- 引入了哪些 **standards** 或 architecture decisions
- shared library 被 **adopted** 的範圍有多廣
- 工作是否減少了 review time、defects、duplication 或 onboarding time
- 如何處理分歧或模糊的技術決策

Senior engineer 是 **multiplier**。Profile 仍需證明能創造清晰度、建立有用的約束，並幫助其他工程師交付更好的工作，而不成為瓶頸。
...
履歷列出 **Cypress**、**Vitest**、**React Testing Library**、**Lighthouse**、**axe**、**Sentry**，以及 agent observability。這證明熟悉有用的工具，但尚未說明 **quality strategy**。
...
Senior signal 不是會寫測試，而是能設計一套 **quality system**，在不讓交付變慢或脆弱的前提下，抓住重要失敗。

<br />

### Accessibility Depth

Profile 目前記錄了 **semantic HTML**、keyboard-accessible 的 navigation 與 forms、可見 focus、有意義的 alt text，以及用 **Lighthouse** 與 **axe** 做自動化檢查。

這是有用的基準，但自動化工具只偵測到 accessibility 問題的一部分。仍需更深入練習：
...
AI 工作已包含 retries、durable runs、traces、cost 與 latency metrics，以及 scored evaluations。下一步是證明這些系統在出錯時仍能安全且可預期地運作。
...
**LLM-as-judge** 有用，但不應是唯一量度。還需要 deterministic checks，以及像 **tool-call success**、**retrieval recall**、**groundedness** 與 end-to-end task completion 這類 task-specific metrics。

<br />

### Applied AI Versus ML Engineering

當前證據在 **applied AI** 與 **agent engineering** 上最強，而不是以模型為中心的 **ML engineering**。

若鎖定偏重 ML 的職位，就需要在 statistics、experimentation、data preparation、model training、fine-tuning 與 model evaluation 上更深。若繼續鎖定 product 與 agent 角色，就應明確說出，並加深讓 AI features 可靠的 **system-design** 技能。

目前，此 profile 的定位是：

> **Senior Frontend Engineer for AI Products** 或 **Applied AI Engineer with deep frontend expertise**。

這比把 profile 呈現為一般 AI 或 ML engineer 更準確、也更有差異化。
...
這個區分避免兩個錯誤。不應花數月重學已經會做、只是沒記錄清楚的事；也不應改寫履歷 bullet，暗示尚未展現的能力。
...
此計劃的首要任務是讓工程推理可見，並建立更強的 quality baseline。
...
將在合適處把 **axe** checks 加入 CI，但不會把自動化綠燈報告視為完整的 accessibility coverage。
...
將建立一套 **failure-oriented evaluation set**，而不是只評估預期成功的 prompts。
...
將把 frontend 與 agent observability 連接到明確的 service objectives。
...
將進行一次 **incident exercise**，並撰寫一份產生具體後續工作的 **blameless postmortem**。
...
此計劃指定擁有一項 engineering standard 或 **RFC**，並透過一段有意義的工作 mentoring 另一位工程師。
...
此計劃要求記錄某個 project 為何使用特定 **model**、**embedding**、**chunking strategy**、**reranker**、cache 或 routing policy。
...
在 12 個月結束時，此計劃的目標是反覆證據，證明能帶領複雜 initiative，而不只是單一孤立例子。
...
5. Mentoring 工程師，並建立即使無需持續介入仍然有用的 standards。
...
將每季覆核這份 scorecard。如果某項活動無法連接到這些結果之一，就應質疑它是否是發展時間的最高價值用途。
...
在工作尚未支撐之前，不應加上主張。履歷應因 scope 與 judgment 成長而變強，而不是因為用詞變得更誇張。
...
此 profile 已擅長跨 frontend、backend、cloud 與 applied AI 建構產品。要成為更強的 senior engineer，此計劃要求刻意轉變：
...
此職業路徑的下一階段，不會由掌握多少 frameworks 定義，而會由決策品質、預防的風險、能運營的系統，以及因團隊貢獻而變得更好的工程師來定義。
