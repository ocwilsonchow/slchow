...
Typed HTTP 基礎見 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/_backend-apis-hono-sst)。可靠性 pattern 見 [System Design 裡的 Message Queues](/notes/message-queues-in-system-design)。Partitioning、offsets 與 replay 見 [System Design 裡的 Kafka](/notes/kafka-in-system-design)。Schema 從 [System Design 裡的 Data Modeling](/notes/data-modeling-in-system-design) 開始。這篇 note 講的是購票路徑。
...
Multi-AZ 處理 AZ failure。它不是 regional disaster recovery。這裡的目標是 **RPO 不到一分鐘** 與 **RTO 不到十五分鐘**。那是可測目標，不是 AWS 產品保證。
