...
便利也伴隨限制。Values 是字串，access 是同步的，每次 read 或 write 都會阻塞 main thread。Capacity 與 eviction 行為也因瀏覽器而異。它是小型 persistence 機制，而不是 database。

<br />

任何在該 origin 上執行的 JavaScript 都能讀取它。這包括透過 XSS 漏洞引入的惡意程式碼，因此 session tokens、passwords 或敏感個人資料不應存在 `localStorage`。長壽命的 bearer tokens 特別危險，因為竊取該值就足以在別處重用。

<br />

stored keys 會做 version，parsed data 會驗證。已部署的應用會變，而舊的 browser data 仍在：
...
它不是安全邊界。頁面上的 scripts 仍然可以讀取它，用戶也能以讓 lifecycle 假設變得不那麼明顯的方式複製或還原 tabs。`sessionStorage` 適用於 **product lifetime** 與資料匹配的場景，而不是因為它比 `localStorage` 更安全。
...
Client-side code 仍然無法透過 `cookieStore` 讀取 `HttpOnly` cookie。這正是限制的意義。一般 application state 也不應放進 cookies：它們很小，domain 與 path 規則很微妙，而且隨 requests 帶上的 cookies 每次都會增加網路開銷。
