...
這篇 note 依 [Evan 的 walkthrough](https://www.youtube.com/watch?v=DQ57zYedMdQ)。Typed Hono 路徑見 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/backend-apis-hono-sst)。這篇 note 講的是 contract。
...
- 這套 stack 真正 ship 的 version 是 OpenAPI contract —— [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/backend-apis-hono-sst)。
