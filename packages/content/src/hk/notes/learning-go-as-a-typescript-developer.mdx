...
這篇 note 是以 **Hono** developer 身分讀 **Go 1.22+** API 時用的 mapping。Samples 對準 standard library：method-aware `ServeMux`、`encoding/json`、`database/sql`。**sqlc** 與 **go-playground/validator** 各出現一次，作為 Drizzle 與 Zod 的表親。**chi** 出現一次，作為 middleware-composition 表親。不是把這個 site 重寫成 Go，也不是 Gin。Typed Hono stack 見 [用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/backend-apis-hono-sst)。Types 是 annotations 直到 HTTP boundary 的 Python 兄弟篇是 [以 TypeScript Developer 身分學 Python](/notes/learning-python-as-a-typescript-developer)。Types 活過 compilation 但 interfaces 是 nominal 的 Java 兄弟篇是 [以 TypeScript Developer 身分學 Java](/notes/learning-java-as-a-typescript-developer)。你已有的 object model 見 [TypeScript Class 與 Runtime Identity](/notes/typescript-classes-and-runtime-identity)。
