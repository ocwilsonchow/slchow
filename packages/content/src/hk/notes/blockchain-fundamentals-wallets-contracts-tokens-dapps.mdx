...
smart contract 是 **一套開放、按 fee 計量的 API，而其 database 就是 chain**。UI 可以說謊；真正 settle 的是 contract 的 state。
...
許多 dApps 仍然使用 centralized pieces：indexers、APIs、IPFS gateways、admin keys。「Decentralized」是一個光譜。重要的是哪些部分可以失敗或審查，而不拿走用戶的 assets。
