...
這篇 note 收集我最常伸手去用的 patterns，每一個都附上精確的 TypeScript 實作與 complexity 說明。目標是一份能用的心智地圖：當你看到一種 input 形狀，就已經知道該用哪把工具。
...
Hash map 把昂貴的 scan 變成 constant-time lookups。當你本來會 nest loops 去找 complement、數 frequencies，或記住「我見過這個嗎？」時，就用它。
