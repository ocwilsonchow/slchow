...
- **Relational.** Tables、foreign keys、ACID。Users、posts、likes。`JOIN` 讓「我 follow 的人發的 posts」是一條 query，不是一個產品。Strong consistency —— payments、inventory、unique email —— 是這套 stack 留在 Postgres 的原因。那句話在做的 trade 見 [System Design 裡的 CAP Theorem](/notes/cap-theorem-in-system-design)。Scale 是 replicas、pooling、caching，然後 sharding —— 不是換一個 database。
