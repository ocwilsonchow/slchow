...
本文集中討論交付流程。應用層風險見 [Web Security and the OWASP Top 10](/notes/web-security-owasp-top-10)，完整的交付管線見 [React 的 Frontend CI/CD](/notes/frontend-ci-cd-for-react)，React Native / Expo 交付見 [用 EAS Workflows 做 React Native CI/CD](/notes/react-native-ci-cd-with-eas-workflows)，以 stage 管理基礎設施則見 [用 SST 管理 AWS 基礎設施與 DevOps](/notes/using-sst-for-aws-infra-and-devops)。
...
Preview 與 artifact promotion 等完整結構見 [Frontend CI/CD 文章](/notes/frontend-ci-cd-for-react)。
...
SBOM 和 provenance 改善可追溯性，但不取代程式碼審查、測試或漏洞管理。React Native 亦包含由 Gradle 和 CocoaPods 管理的 native dependencies，因此供應鏈審查不能只檢查 JavaScript dependency graph。詳見 [React Native 裡的 Security](/notes/security-in-react-native)。把 Build / Run / Verify 套用到 EAS Workflows、stage secrets 與 OTA promotion，見 [用 EAS Workflows 做 React Native CI/CD](/notes/react-native-ci-cd-with-eas-workflows)。
