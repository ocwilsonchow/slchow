...
- 強制 `customers.email` 的 unique index 既是 constraint 也是 access path。`orders.customer_id` 上的 plain index 只是 access path。
...
- Isolation、RLS 與 tenant context：[用 Hono、Better Auth、Drizzle 與 Postgres RLS 打造 Multi-Tenant 後端](/notes/multi-tenant-backend-drizzle-hono-better-auth)。Typed schema、pool 與 migrations：[用 Hono、Drizzle、Zod OpenAPI 與 SST 打造 Backend APIs](/notes/backend-apis-hono-sst)。
