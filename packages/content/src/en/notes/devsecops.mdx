...
This note focuses on the delivery process. Application-level risks are covered in [Web Security and the OWASP Top 10](/notes/web-security-owasp-top-10), the broader delivery pipeline is described in [Frontend CI/CD for React](/notes/frontend-ci-cd-for-react), React Native / Expo delivery is [React Native CI/CD with EAS Workflows](/notes/react-native-ci-cd-with-eas-workflows), and stage-based infrastructure is covered in [Using SST to Manage AWS Infrastructure and DevOps](/notes/using-sst-for-aws-infra-and-devops).
...
The detailed CI/CD structure, including previews and artifact promotion, is covered in the [Frontend CI/CD note](/notes/frontend-ci-cd-for-react).
...
SBOMs and provenance improve traceability; they do not replace code review, testing, or vulnerability management. React Native also includes native dependencies managed through Gradle and CocoaPods, so its supply-chain review must cover more than the JavaScript dependency graph. See [Security in React Native](/notes/security-in-react-native). Applying Build / Run / Verify to EAS Workflows, stage secrets, and OTA promotion is [React Native CI/CD with EAS Workflows](/notes/react-native-ci-cd-with-eas-workflows).
