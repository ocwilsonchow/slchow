...
The typed API through production protect, monitor, and recover is [Backend APIs with Hono, Drizzle, Zod OpenAPI, and SST](/notes/_backend-apis-hono-sst). Stages and the deploy loop are [Using SST to Manage AWS Infrastructure and DevOps](/notes/using-sst-for-aws-infra-and-devops). A concrete outbox, inbox, and compensation flow is [Building an Event-Driven Ticketing Backend](/notes/building-an-event-driven-ticketing-backend). This note is the queue.
...
| [**Kafka / Kinesis**](/notes/kafka-in-system-design) | Replayable log | Multiple independent readers, ordered history, replay. |
