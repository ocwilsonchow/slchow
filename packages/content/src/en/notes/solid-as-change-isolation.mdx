...
```ts title="Failure — first draft: copy, send, and vendor in one function"
...
```ts title="Keep — one actor per module"
...
```ts title="Failure — split by line count, not by actor"
...
```ts title="Keep — split by who asks, not by how many lines"
export function formatNotification(event: SignupEvent): Notification {
  if (event.type === "welcome") {
    return {
      to: event.user.email,
...
```ts title="Failure — dispatcher grows with every channel"
...
```ts title="Keep — dispatcher closed for modification"
...
```ts title="Keep — register channels at the composition root"
...
```ts title="Failure — silent Noop breaks the Deliver contract"
...
```ts title="Keep — recording fake honors the Deliver contract"
...
```ts title="Failure — throw when the type says Result"
...
A fake that records messages and returns the same `DeliverResult` is substitutable. A fake that `throw`s when the type says Result is not — that half is [TypeScript Error Handling](/notes/typescript-error-handling).
...
```ts title="Failure — fat port forces stubs for unused methods"
...
```ts title="Failure — one message type stuffed for every channel"
export type Notification = {
  to: string
  subject: string
  body: string
...
```ts title="Keep — Deliver stays small; push is a different port"
export type Notification = {
  to: string
  subject: string
  body: string
...
```ts title="Failure — use case imports the vendor SDK"
import { Resend } from "resend"
...
```ts title="Keep — use case depends on Deliver, not Resend"
export async function notify(
  event: SignupEvent,
  deliver: Deliver,
): Promise<DeliverResult> {
...
```ts title="Keep — concrete SDK stays in the adapter factory"
import { Resend } from "resend"
...
```ts title="Keep — composition root wires concretes to the use case"
// composition root
const resend = new Resend(process.env.RESEND_API_KEY)
