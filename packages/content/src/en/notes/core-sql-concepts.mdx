...
- The unique index that enforces `customers.email` is both constraint and access path. A plain index on `orders.customer_id` is only an access path.
...
- Isolation, RLS, and tenant context: [Building a Multi-Tenant Backend with Hono, Better Auth, Drizzle, and Postgres RLS](/notes/multi-tenant-backend-drizzle-hono-better-auth). Typed schema, pool, and migrations: [Backend APIs with Hono, Drizzle, Zod OpenAPI, and SST](/notes/backend-apis-hono-sst).
