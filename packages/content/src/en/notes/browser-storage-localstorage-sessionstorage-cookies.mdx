...
The convenience comes with limits. Values are strings, access is synchronous, and every read or write blocks the main thread. Capacity and eviction behavior also vary by browser. It is a small persistence mechanism, not a database.

<br />

Any JavaScript running on the origin can read it. That includes malicious code introduced through an XSS vulnerability, so session tokens, passwords, or sensitive personal data do not belong in `localStorage`. Long-lived bearer tokens are especially risky because stealing the value is enough to reuse it elsewhere.

<br />

Stored keys are versioned and parsed data is validated. Deployed applications change while old browser data remains:
...
It is not a security boundary. Scripts on the page can still read it, and users can duplicate or restore tabs in ways that make lifecycle assumptions less obvious. `sessionStorage` fits when its **product lifetime** matches the data, not because it is safer than `localStorage`.
...
Client-side code still cannot read an `HttpOnly` cookie through `cookieStore`. That restriction is the point. General application state also does not belong in cookies: they are small, domain and path rules are subtle, and cookies included with requests add network overhead every time.
