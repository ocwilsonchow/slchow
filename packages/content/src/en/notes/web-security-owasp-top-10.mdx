...
The [OWASP Top 10:2025](https://owasp.org/Top10/2025/) is a map of how those boundaries fail, not a checklist to memorize. This note walks each category as *what fails*, *what an attacker is trying to achieve*, and *how it is defended* in a **Next.js**, **Hono**, **Better Auth**, **Drizzle**, and **PostgreSQL** stack.
...
OpenAPI and Scalar are also stage-gated in production. A public reference is free reconnaissance. Cloud buckets, database security groups, and leftover `/debug` routes are the same review: if it is reachable and not required, it is open.
...
- Access tokens do not belong in `localStorage`. An XSS on the origin can read it, and unlike an `HttpOnly` cookie the value is easy to replay from another client. Opaque session cookies with `HttpOnly; Secure; SameSite` are the default. The storage note covers why.

Application code does not invent crypto. Token binding, envelope encryption, and key rotation are library problems. Application code should call a maintained primitive and keep the key out of the repo.
...
String-built SQL is the failure mode. It is not shown here. If a raw fragment is unavoidable, it is a constant owned by the codebase, never a request field.

On the HTML side, React already escapes text children. The remaining holes are `dangerouslySetInnerHTML`, `href` values that can become a script URL, and Markdown or rich-text pipelines that emit raw HTML. User content stays as data, HTML is sanitized through a maintained library if the product truly needs markup, and CSP sits behind it.
...
Better Auth already owns password hashing, session records, and cookie issuance. The session stays in a cookie the page script cannot read:
...
`HttpOnly` keeps XSS from trivially exporting the session. `Secure` keeps it off HTTP. `SameSite=Lax` is the default starting point for cookie-based CSRF reduction on ordinary navigations; mutating cross-site POSTs need a stricter analysis (SameSite, origin checks, or anti-CSRF tokens) depending on the cookie and the API layout.
...
Application code does not `eval`, `new Function`, or deserialize into objects that can carry behavior. Pin container image digests in deploy config. Treat CI outputs as untrusted until the same pipeline that signed them produced them.
...
Security-relevant events are logged, not request bodies:
...
ORM messages, Postgres error codes, and stack traces are never returned to the client. Those are logs. The user-facing body is a stable `code` plus a generic message.
