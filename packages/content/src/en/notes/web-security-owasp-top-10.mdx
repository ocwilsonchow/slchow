...
```text
Browser origin → Cookie / session → Hono API → Authn then authz → Postgres
```
...
- **CSP** is a backup for XSS, not a substitute for not rendering untrusted HTML. The SST API Gateway example in [Backend APIs with Hono, Drizzle, Zod OpenAPI, and SST](/notes/_backend-apis-hono-sst) already names the app origin.
...
- **Pin the supply chain.** Lockfile in git, frozen installs, Actions pinned to SHAs, secrets in SST not in the repo. The pipeline that enforces that is [DevSecOps](/notes/devsecops).
