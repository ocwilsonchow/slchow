...
Security cuts across runtime, concurrency, networks, databases, and distributed systems
```
...
- How bytes become a first paint is [The Critical Rendering Path in the Browser](/notes/critical-rendering-path-in-the-browser). The API and edge shape deployed in this stack is [Backend APIs with Hono, Drizzle, Zod OpenAPI, and SST](/notes/backend-apis-hono-sst). The HTTP contract is [API Design in System Design](/notes/api-design-in-system-design). Cookie lifetime and `HttpOnly` belong with [Local Storage, Session Storage, and Cookies](/notes/browser-storage-localstorage-sessionstorage-cookies).
...
- The isolation path is [Building a Multi-Tenant Backend with Hono, Better Auth, Drizzle, and Postgres RLS](/notes/multi-tenant-backend-drizzle-hono-better-auth). The statement model is [Core SQL Concepts](/notes/core-sql-concepts). The schema itself is [Data Modeling in System Design](/notes/data-modeling-in-system-design). The read path in front of Postgres is [Caching in System Design](/notes/caching-in-system-design).
...
- The class of failures is [Web Security and the OWASP Top 10](/notes/web-security-owasp-top-10). The two hosts in this stack are [Security in Next.js](/notes/security-in-next-js) and [Security in React Native](/notes/security-in-react-native). Tenant isolation as a database constraint is the multi-tenant note above.
