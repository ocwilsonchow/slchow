...
description: How I isolate tenants with Better Auth organizations, Hono request context, and Postgres row-level security on a shared Drizzle schema
date: 2026-08-06
---
...
This note assumes you already have a typed backend with **Hono**, **Better Auth** sessions, **Drizzle**, and **PostgreSQL**. I covered that base stack in [How I Build Backend APIs with Hono, Drizzle and Zod OpenAPI](/notes/how-i-build-backend-apis). Here the focus is **tenant isolation**.
...
I use a **shared database** and a **shared schema**. Every tenant-owned row carries an `organization_id`. That is the common SaaS model, and it fits **Better Auth**'s organization plugin directly.
...
Non-goals for this note: schema-per-tenant, database-per-tenant, and billing or metering. Those are different architectures. Teams and dynamic access control exist in Better Auth; I keep v1 on **static roles**.
...
If you use the API Gateway Lambda authorizer pattern from the base note, pass only small identity identifiers such as `userId` and `sessionId`. The business route still supplies `organizationId`, and the API verifies membership for that exact organization before opening the RLS transaction. Never copy identity or tenant from a client-controlled header without that verification.
...
The **organization** plugin is the membership and workspace layer. I install `@better-auth/drizzle-adapter`, enable the plugin on the server and client, then generate and apply a migration so `organization`, `member`, `invitation`, and `session.activeOrganizationId` exist in PostgreSQL.
...
The invitation callback must deliver the opaque invitation ID only to the intended recipient. Acceptance requires an authenticated session whose email matches the invitation. I also require email verification and disable direct organization deletion so deletion goes through an explicit archival workflow.
...
The first command writes schema definitions; it does **not** change PostgreSQL. In CI I pin the CLI to the Better Auth version used by the application instead of allowing `@latest` to move independently.
...
Default roles are **owner**, **admin**, and **member**. For domain actions I define an access controller and pass it into the plugin:
...
Teams and dynamic access control are available when you outgrow static roles. I do not enable them on day one.
...
Better Auth organization IDs are strings, so domain foreign keys should be `text`, not `uuid`, unless you have customized ID generation.
...
Adding a policy enables RLS on the table in Drizzle. If you need RLS with no policies yet, use `pgTable.withRLS(...)` — with no policy, Postgres default-denies row access.
...
I provision the product runtime role outside Drizzle, keep it from owning schemas or tables, and add grants plus `FORCE` in a reviewed custom migration:
...
The login credential is supplied by infrastructure or a secret manager, not committed in a migration. Policies do not grant table privileges, so each migration grants only the DML the runtime needs. I do not grant `TRUNCATE`, DDL, broad `REFERENCES`, or role-management privileges. RLS does not govern `TRUNCATE`, and unique/foreign-key checks bypass row filtering, so I scope constraints by tenant and avoid exposing raw conflict details. If a future table uses identity or serial sequences, I review its sequence grants separately.

Migration discipline still matters. For an existing table I expand and contract: add the tenant column, backfill it, enforce `NOT NULL`, add policies and grants, verify them under `app_user`, then deploy code that depends on the boundary. Drizzle role discovery such as `entities.roles` does not replace custom SQL for attributes, grants, membership, default privileges, or `FORCE RLS`.

---

## 5. Postgres RLS with Drizzle

RLS is useful only if the **request path** connects as a restricted role. I keep separate credentials:
...
Transaction-local settings matter with **pooled connections**. Neon, PgBouncer, and warm Lambda environments reuse connections. `set_config(..., true)` is reverted by PostgreSQL at commit or rollback; I do not issue reset SQL in `finally`, because a failed statement leaves the transaction aborted and cleanup SQL can mask the original error. I never define `app.organization_id` with `ALTER ROLE`, `ALTER DATABASE`, or session-level `SET`, and I do not depend on pool reset hooks.

I still write explicit filters in application code:
...
The test checklist I want before calling the system production-ready:
...
`getActiveMemberRole` throws a Better Auth `APIError` when the organization or membership is invalid; it does not return `null`. I map expected membership failures to a safe `403` and let unexpected errors reach the centralized error handler.
...
- **Lambda authorizer caching** — identity can be cached only within the revocation guarantees you accept. Tenant membership and permission are still checked against the explicit route organization in the API.
...
**Better Auth** organizations give you membership UX: create, invite, roles, active workspace. **Postgres RLS** gives you a data plane that does not trust every query author to remember the tenant filter. **Hono** is the place those two meet — typed context, explicit middleware, and handlers that open a tenant-scoped transaction before touching business tables.

Start with a shared schema, static roles, and RLS on every tenant-owned table. Add teams or dynamic access control when a real product requirement appears. Keep deploy, OpenAPI, and Lambda details in the [base backend note](/notes/how-i-build-backend-apis); keep this note about the isolation contract.
