...
That queue sat on a tighter coupling: React Native was written against **JavaScriptCore**'s C API. The engine call sites were verbose, they managed memory by hand, and swapping engines meant forking the framework. **JSI** (JavaScript Interface) is the C++ wrapper that broke that coupling. Hermes, JSC, V8, and Chakra can each implement the same Runtime. Hermes exists because of this seam.

The New Architecture replaces the queue with **JSI**, the renderer with **Fabric**, and native modules with **TurboModules** (and, in Expo, the **Expo Modules API**). Expo SDK 55 dropped the legacy architecture. SDK 57 does not offer it as a runtime option.

JSI lets a JavaScript engine hold a reference to a native object, and native code hold a reference to a JavaScript function. A JS call into native can be **synchronous**. There is no JSON round-trip on the hot path. Updates written in C++ shared memory are visible to JavaScript immediately.

The Runtime is a first-class object, not a slogan. Almost every JSI call takes one. It may be read and written only on the **JS thread**; off-thread access is undefined behavior. The framework owns its lifecycle and can tear it down and restart it. Capturing it means you own that lifecycle. It is not a singleton.
...
- A **host function** is a C++ function installed into JS. Early JSI native modules used this path: `jsi::Value` → a C++ `dynamic` (a generic bag) → native types. A **host object** is a C++ object whose `get` / `set` run when JS reads a property. TurboModules, the Fabric UIManager, and Expo Modules are host objects.
- Fabric is faster because it dropped deserialization hops, not because JSI is magic. The legacy UIManager was a native module: dynamic → generic prop map → another deserialize → native view. Fabric's UIManager is a host object: wrap the JSI value as RawProps → convert once to native props.
- iOS wins more of that hop because Objective-C++ can bridge JSI values to native types more directly. Android still pays **JNI** between Java/Kotlin and C++. JSI is not a silver bullet.
...
The word "bridge" still appears in blog posts and in interop layers for old modules. It is not the contract the framework is built on. These Runtime and host-object names are implementation observations. Phillip Pan's [React Native under the hood](https://www.youtube.com/watch?v=oLmGInjKU2U) (App.js Conf 2024) walks them.
...
- **CallInvoker** is how native code hops **onto the JS thread** from the UI thread or a background thread. The dispatch is asynchronous. It does not let you block the current thread until JS finishes. That is why camera and animation libraries still need their own UI-thread worklets rather than calling JSI from the UI thread.
...
Expo's **Expo Modules API** sits on the same JSI layer. Its Swift and Kotlin DSL compiles to JSI host bindings. `expo-secure-store`, `expo-camera`, `expo-font`, and `expo-image` are native modules with JavaScript entry points. They are not optional wrappers around `fetch`. The screen still calls a documented module, not `jsi::Runtime`.

C++ TurboModules receive a Runtime because their methods run on the JS thread by default. That is an implementation observation, not a reason to install raw bindings from a screen.
...
- **React Native is not a WebView.** A WebView is a browser origin inside the app. Fabric mounts platform views. Mixing the two is a security boundary, covered in [Security in React Native](/notes/security-in-react-native).
...
- **JSI does not mean zero serialization.** Early JSI modules still converted through a C++ `dynamic`. Android still pays JNI between Java/Kotlin and C++.
- **Do not stash the Runtime.** It is JS-thread only. The framework can tear it down and restart it.
...
For React's own queues, lanes, and commit phase, continue with [Understanding React in Depth](/notes/understanding-react-in-depth). For the web contrast, see [The Critical Rendering Path in the Browser](/notes/critical-rendering-path-in-the-browser). For Next.js as the analogous orchestrator on the server/document side, see [Understanding Next.js in Depth](/notes/understanding-next-js-in-depth). For the device threat model (extractable bundle, SecureStore, WebView bridges), see [Security in React Native](/notes/security-in-react-native).

Primary references: Expo's [SDK 57 changelog](https://expo.dev/changelog/sdk-57); React Native's [architecture overview](https://reactnative.dev/architecture/overview), [render pipeline](https://reactnative.dev/architecture/render-pipeline), [threading model](https://reactnative.dev/architecture/threading-model), and [New Architecture](https://reactnative.dev/docs/the-new-architecture/landing-page) notes; and Phillip Pan's [React Native under the hood](https://www.youtube.com/watch?v=oLmGInjKU2U) (App.js Conf 2024) for JSI Runtime, host functions, and host objects. Public Expo and React Native contracts are the durable API. Shadow-node internals and Metro details are debugging evidence for today's toolchain, not an API to hard-code.
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="rn-jsi-bridge">
    ...
    <AccordionContent>
      ...
      - The legacy Bridge was a **message bus**: JSON over an async queue, on a framework coupled to JSC's C API. The New Architecture is the default in this SDK line.
      - **JSI** is a C++ API that lets the engine hold a native object and native code hold a JS function. A JS call into native can be **synchronous**. There is no JSON round-trip on the hot path. Hermes exists because JSI made the engine swappable.
      - The Runtime is JS-thread only and framework-owned. It is not a singleton. Off-thread access is undefined behavior.
      - Early JSI modules were **host functions** that still converted through a C++ `dynamic`. TurboModules, the Fabric UIManager, and Expo Modules are **host objects**: `get` / `set` run when JS reads a property, and Fabric converts RawProps once instead of deserializing twice.
      - iOS wins more of that hop. Android still pays **JNI**. JSI is not a silver bullet.
      ...
      - Synchronous does not mean free. A heavy JSI call on the **JS thread** still stalls React. If the work belongs on the UI thread, the module hops via **CallInvoker** (async, non-blocking) or a UI-thread worklet.
  ...
  <AccordionItem value="rn-threading">
    ...
    <AccordionContent>
      ...
      - **CallInvoker** hops onto the JS thread asynchronously. It cannot block the current thread until JS finishes.
