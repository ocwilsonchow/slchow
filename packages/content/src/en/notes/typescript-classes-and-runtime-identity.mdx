...
- Type-level `extends` — a constraint, a ternary, `infer` — is [TypeScript Infer, Extends, and Ternaries](/notes/typescript-infer-extends-and-ternaries). This section is the prototype chain.
- `noImplicitOverride` forces the `override` keyword when a child method replaces a parent method. That flag is [TypeScript Beyond Strict](/notes/typescript-beyond-strict).
...
- Constructor parameter properties (`constructor(private name: string)`) and `enum` **emit values**. `erasableSyntaxOnly` forbids them so types strip clean. That flag is [TypeScript Beyond Strict](/notes/typescript-beyond-strict).
...
- A string union on a result type is often better when the failure is a **return** value. That half is [TypeScript Error Handling](/notes/typescript-error-handling).
...
- A Hono handler is a function from context to response. Nest-style controller classes belong to a different stack. [Backend APIs with Hono](/notes/_backend-apis-hono-sst) keeps routes as functions.
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="ts-error-subclasses">
    ...
    <AccordionContent>
      ...
      - A string union on a result type is often better when the failure is a **return** value. That half is [TypeScript Error Handling](/notes/typescript-error-handling). Once something is `throw`n, the prototype chain is the test that survives.
