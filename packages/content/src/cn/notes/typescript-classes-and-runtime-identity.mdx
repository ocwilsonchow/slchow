...
- Type-level `extends` —— constraint、ternary、`infer` —— 见 [TypeScript Infer、Extends 与 Ternaries](/notes/typescript-infer-extends-and-ternaries)。这节讲的是 prototype chain。
- `noImplicitOverride` 要求 child method 取代 parent method 时写 `override`。那个 flag 见 [TypeScript Beyond Strict](/notes/typescript-beyond-strict)。
...
- Constructor parameter properties（`constructor(private name: string)`）与 `enum` 会 **emit values**。`erasableSyntaxOnly` 禁止它们，好让 types 能干净地 strip。那个 flag 见 [TypeScript Beyond Strict](/notes/typescript-beyond-strict)。
...
- Failure 是 **return** value 时，result type 上的 string union 往往更好。那一半见 [TypeScript Error Handling](/notes/typescript-error-handling)。
...
- Hono handler 是从 context 到 response 的 function。Nest 风格的 controller classes 属于另一套 stack。[用 Hono 打造 Backend APIs](/notes/_backend-apis-hono-sst) 把 routes 保持为 functions。
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="ts-error-subclasses">
    ...
    <AccordionContent>
      ...
      - Failure 是 **return** value 时，result type 上的 string union 往往更好。那一半见 [TypeScript Error Handling](/notes/typescript-error-handling)。一旦被 `throw`，能活下来的测试是 prototype chain。
