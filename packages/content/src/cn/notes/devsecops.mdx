...
本文集中讨论交付流程。应用层风险见 [Web Security and the OWASP Top 10](/notes/web-security-owasp-top-10)，完整的交付管线见 [React 的 Frontend CI/CD](/notes/frontend-ci-cd-for-react)，React Native / Expo 交付见 [用 EAS Workflows 做 React Native CI/CD](/notes/react-native-ci-cd-with-eas-workflows)，以 stage 管理基础设施则见 [用 SST 管理 AWS 基础设施与 DevOps](/notes/using-sst-for-aws-infra-and-devops)。
...
Preview 与 artifact promotion 等完整结构见 [Frontend CI/CD 文章](/notes/frontend-ci-cd-for-react)。
...
SBOM 和 provenance 改善可追溯性，但不取代代码审查、测试或漏洞管理。React Native 也包含由 Gradle 和 CocoaPods 管理的 native dependencies，因此供应链审查不能只检查 JavaScript dependency graph。详见 [React Native 里的 Security](/notes/security-in-react-native)。把 Build / Run / Verify 套用到 EAS Workflows、stage secrets 与 OTA promotion，见 [用 EAS Workflows 做 React Native CI/CD](/notes/react-native-ci-cd-with-eas-workflows)。
