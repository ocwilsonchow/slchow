...
- Layout-aware parse 也可以是本地的 **Docling**。Workflow 仍把 parse 当 deterministic step。那份 converter 是 [Docling](/notes/docling)。
...
| Mastra | LangGraph |
| --- | --- |
| `createWorkflow` + `createStep` | `StateGraph` nodes 与 edges |
| Step `execute` | 返回 partial state update 的 node |
| `runId`、snapshots、per-step retries | `thread_id` + checkpointer + `retryPolicy` |
| `requestContext` org | 由 server 组装的 invoke input |
...
Nodes 返回 partial updates；graph 负责套用。`loadFile` 返回 `Command`——state update 与 routing 合并成一次 return——所以它**没有**对外的 static edge。
...
- `loadFile` 用 `Command` 路由，所以它没有对外的 `addEdge`。若照样加一条，两个目标**都会**跑——skip path 会去 parse 一个不存在的 file。
...
<Accordion type="single" collapsible>
  <AccordionItem value="rag-two-pipelines">
    ...
    <AccordionContent>
      ...
      - 同一条 DAG 也可以组成 LangGraph `StateGraph`：nodes 取代 steps，checkpointer 的 `thread_id`（`organizationId:path`）取代 `runId`，`Send` 把分批 upsert 并行扇出。Invariants 不变——org 照样来自 server，永不来自 model。
