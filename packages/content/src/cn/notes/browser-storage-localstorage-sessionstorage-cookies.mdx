...
便利也伴随限制。Values 是字符串，access 是同步的，每次 read 或 write 都会阻塞 main thread。Capacity 与 eviction 行为也因浏览器而异。它是小型 persistence 机制，而不是 database。

<br />

任何在该 origin 上执行的 JavaScript 都能读取它。这包括通过 XSS 漏洞引入的恶意代码，因此 session tokens、passwords 或敏感个人数据不应存在 `localStorage`。长寿命的 bearer tokens 特别危险，因为窃取该值就足以在别处重用。

<br />

stored keys 会做 version，parsed data 会验证。已部署的应用会变，而旧的 browser data 仍在：
...
它不是安全边界。页面上的 scripts 仍然可以读取它，用户也能以让 lifecycle 假设变得不那么明显的方式复制或还原 tabs。`sessionStorage` 适用于 **product lifetime** 与数据匹配的场景，而不是因为它比 `localStorage` 更安全。
...
Client-side code 仍然无法通过 `cookieStore` 读取 `HttpOnly` cookie。这正是限制的意义。一般 application state 也不应放进 cookies：它们很小，domain 与 path 规则很微妙，而且随 requests 带上的 cookies 每次都会增加网络开销。
