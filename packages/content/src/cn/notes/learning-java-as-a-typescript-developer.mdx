...
Hono 是你用 request 调用的 function。Spring Boot 是一个 **建构 beans 的 process**，再把 HTTP map 到 methods。那个分裂的 NestJS 表亲 —— TypeScript container，外围 Express 或 Fastify —— 见 [深入理解 NestJS](/notes/understanding-nestjs-in-depth)。
