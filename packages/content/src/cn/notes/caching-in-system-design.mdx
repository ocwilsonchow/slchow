...
- **Cost of compute.** 把 posts、follows、likes join 起来的 personalized feed。把结果 cache 六十秒。不要每次 scroll 都重算。Denormalized feed 是 cache；tables 保持 normalized。[System Design 里的 Data Modeling](/notes/data-modeling-in-system-design)。
