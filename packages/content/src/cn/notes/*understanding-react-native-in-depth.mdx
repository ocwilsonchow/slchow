...
那条 queue 坐在更紧的耦合上：React Native 是对着 **JavaScriptCore** 的 C API 写的。Engine call sites 冗长、要手管 memory，换 engine 意味着 fork framework。**JSI**（JavaScript Interface）是打破那层耦合的 C++ wrapper。Hermes、JSC、V8 与 Chakra 都可以实现同一份 Runtime。Hermes 存在，是因为有这条缝。
...
JSI 让 JavaScript engine 持有 native object 的 reference，也让 native code 持有 JavaScript function 的 reference。JS 对 native 的 call 可以是 **synchronous**。Hot path 上没有 JSON round-trip。写进 C++ shared memory 的 updates，JavaScript 立刻可见。

Runtime 是一等对象，不是口号。几乎每次 JSI call 都要带一份。它只能在 **JS thread** 上读和写；离 thread 访问是 undefined behavior。Framework 拥有它的 lifecycle，可以 tear down 再 restart。Capture 它意味着你自己管那份 lifecycle。它不是 singleton。
...
- **Host function** 是装进 JS 的 C++ function。早期 JSI native modules 走这条路：`jsi::Value` → C++ `dynamic`（一只 generic bag）→ native types。**Host object** 是 C++ object，JS 读 property 时跑它的 `get` / `set`。TurboModules、Fabric UIManager 与 Expo Modules 都是 host objects。
- Fabric 更快，是因为它丢掉了 deserialization hops，不是因为 JSI 有魔法。Legacy UIManager 是 native module：dynamic → generic prop map → 再 deserialize 一次 → native view。Fabric 的 UIManager 是 host object：把 JSI value wrap 成 RawProps → 一次转成 native props。
- iOS 多赢这一 hop，因为 Objective-C++ 能更直接地把 JSI values bridge 到 native types。Android 仍要在 Java/Kotlin 与 C++ 之间付 **JNI**。JSI 不是银弹。
...
「Bridge」这个词仍出现在 blog posts 与旧 modules 的 interop layers 里。它不是 framework 所建立的契约。这些 Runtime 与 host-object 名字是实现观察。Phillip Pan 的 [React Native under the hood](https://www.youtube.com/watch?v=oLmGInjKU2U)（App.js Conf 2024）走一遍它们。
...
- **CallInvoker** 是 native code 从 UI thread 或 background thread **hop 上 JS thread** 的方式。Dispatch 是 asynchronous。它不让你 block 当前 thread 等到 JS 跑完。这就是 camera 与 animation libraries 仍需要自己的 UI-thread worklets，而不是从 UI thread 调 JSI 的原因。
...
Expo 的 **Expo Modules API** 坐在同一层 JSI 上。它的 Swift 与 Kotlin DSL compile 成 JSI host bindings。`expo-secure-store`、`expo-camera`、`expo-font` 与 `expo-image` 是带 JavaScript entry points 的 native modules。它们不是 `fetch` 外面的可选 wrappers。Screen 仍调用已文档化的 module，不是 `jsi::Runtime`。

C++ TurboModules 收到一份 Runtime，因为它们的 methods 默认跑在 JS thread 上。那是实现观察，不是从 screen 安装 raw bindings 的理由。
...
- **React Native 不是 WebView。** WebView 是应用里的 browser origin。Fabric mount platform views。混用两者是 security boundary，见 [React Native 里的 Security](/notes/security-in-react-native)。
...
- **JSI 不代表零 serialization。** 早期 JSI modules 仍经过 C++ `dynamic` 转换。Android 仍要在 Java/Kotlin 与 C++ 之间付 JNI。
- **不要 stash Runtime。** 它只属于 JS thread。Framework 可以 tear down 再 restart。
...
至于 React 自己的 queues、lanes 与 commit phase，继续看 [深入理解 React](/notes/understanding-react-in-depth)。Web 对照见 [Browser 中的 Critical Rendering Path](/notes/critical-rendering-path-in-the-browser)。Server/document 侧对应的 orchestrator 见 [深入理解 Next.js](/notes/understanding-next-js-in-depth)。Device threat model（extractable bundle、SecureStore、WebView bridges）见 [React Native 里的 Security](/notes/security-in-react-native)。

主要参考：Expo 的 [SDK 57 changelog](https://expo.dev/changelog/sdk-57)；React Native 的 [architecture overview](https://reactnative.dev/architecture/overview)、[render pipeline](https://reactnative.dev/architecture/render-pipeline)、[threading model](https://reactnative.dev/architecture/threading-model) 与 [New Architecture](https://reactnative.dev/docs/the-new-architecture/landing-page) notes；以及 Phillip Pan 的 [React Native under the hood](https://www.youtube.com/watch?v=oLmGInjKU2U)（App.js Conf 2024），讲 JSI Runtime、host functions 与 host objects。公开的 Expo 与 React Native 契约是耐用的 API。Shadow-node internals 与 Metro 细节是今天 toolchain 的 debugging 证据，不是要 hard-code 的 API。
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="rn-jsi-bridge">
    ...
    <AccordionContent>
      ...
      - Legacy Bridge 是一条 **message bus**：JSON 走 async queue，坐在耦合 JSC C API 的 framework 上。New Architecture 是这个 SDK line 的默认。
      - **JSI** 是一个 C++ API，让 engine 持有 native object，也让 native code 持有 JS function。JS 对 native 的 call 可以是 **synchronous**。Hot path 上没有 JSON round-trip。Hermes 存在，是因为 JSI 让 engine 可换。
      - Runtime 只属于 JS thread，由 framework 拥有。它不是 singleton。离 thread 访问是 undefined behavior。
      - 早期 JSI modules 是 **host functions**，仍经过 C++ `dynamic` 转换。TurboModules、Fabric UIManager 与 Expo Modules 是 **host objects**：JS 读 property 时跑 `get` / `set`，Fabric 把 RawProps 转一次，而不是 deserialize 两次。
      - iOS 多赢这一 hop。Android 仍付 **JNI**。JSI 不是银弹。
      - Fabric 用这种方式创建 shadow nodes。VisionCamera 可以处理 frames，而不把约 30 MB 的 pixels 每秒六十次复制进 JSON。
      - Synchronous 不代表免费。JS thread 上一次重的 JSI call 仍会 stall React。若工作属于 UI thread，module 经 **CallInvoker**（async、non-blocking）或 UI-thread worklet hop 过去。
  ...
  <AccordionItem value="rn-threading">
    ...
    <AccordionContent>
      ...
      - **CallInvoker** 异步 hop 上 JS thread。它不能 block 当前 thread 等到 JS 跑完。
