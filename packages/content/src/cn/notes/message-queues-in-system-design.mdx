...
Typed API 一路到 production 的 protect、monitor 与 recover 见 [用 Hono、Drizzle、Zod OpenAPI 与 SST 打造 Backend APIs](/notes/_backend-apis-hono-sst)。Stages 与 deploy loop 见 [用 SST 管理 AWS 基础设施与 DevOps](/notes/using-sst-for-aws-infra-and-devops)。具体的 outbox、inbox 与 compensation 流程见 [打造 Event-Driven 票务 Backend](/notes/building-an-event-driven-ticketing-backend)。这篇 note 讲的是 queue。
