...
测试写在 Mangum **之下**。`TestClient` 对同一个 `app` object 说 HTTP。它不需要 AWS event、container，或真实 issuer。Stub `require_user`，让缺失的 JWKS key 不成为 unit-test 的问题。Queries 与 migrations 仍需要可丢弃的 Postgres —— 那是另一套 suite。
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="fastapi-pytest">
    <AccordionTrigger>6. 如何用 pytest 对 FastAPI app 做单元测试？</AccordionTrigger>
    <AccordionContent>

      - 测试写在 Mangum **之下**，用 `TestClient` 打同一个 `app`。命令是 `uv run pytest`。不需要 AWS event、container，或真实 issuer。
      - 经 `dependency_overrides` stub `require_user`。那是 seam。write path 不能碰 Postgres 时，再 override `get_session`。
      - 断言 contract：`/api/health` 是 public，坏 body 是 `400` + `VALIDATION_ERROR`，缺失 bearer 是 **403**（`HTTPBearer`）。
      - ORM → Pydantic 的 mapping 放在 service function，不经 HTTP 也能测。`select`、unique violations 与 Alembic 用可丢弃的 Postgres —— 仍是 pytest，不是 route unit test。
      - **Failure:** 在每个 test 里拼 API Gateway event 再调用 `handler`。那测的是 Mangum，不是 API。
