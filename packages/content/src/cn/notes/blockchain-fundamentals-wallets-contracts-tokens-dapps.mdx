...
smart contract 是 **一套开放、按 fee 计量的 API，而其 database 就是 chain**。UI 可以说谎；真正 settle 的是 contract 的 state。
...
许多 dApps 仍然使用 centralized pieces：indexers、APIs、IPFS gateways、admin keys。「Decentralized」是一个光谱。重要的是哪些部分可以失败或审查，而不拿走用户的 assets。
