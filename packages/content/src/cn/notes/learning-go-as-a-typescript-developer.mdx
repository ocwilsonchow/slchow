...
这篇 note 是以 **Hono** developer 身份读 **Go 1.22+** API 时用的 mapping。Samples 对准 standard library：method-aware `ServeMux`、`encoding/json`、`database/sql`。**sqlc** 与 **go-playground/validator** 各出现一次，作为 Drizzle 与 Zod 的表亲。**chi** 出现一次，作为 middleware-composition 表亲。不是把这个 site 重写成 Go，也不是 Gin。Typed Hono stack 见 [用 Hono、Drizzle、Zod OpenAPI 与 SST 打造 Backend APIs](/notes/_backend-apis-hono-sst)。Types 是 annotations 直到 HTTP boundary 的 Python 兄弟篇是 [以 TypeScript Developer 身份学 Python](/notes/learning-python-as-a-typescript-developer)。Types 活过 compilation 但 interfaces 是 nominal 的 Java 兄弟篇是 [以 TypeScript Developer 身份学 Java](/notes/learning-java-as-a-typescript-developer)。你已有的 object model 见 [TypeScript Class 与 Runtime Identity](/notes/typescript-classes-and-runtime-identity)。
