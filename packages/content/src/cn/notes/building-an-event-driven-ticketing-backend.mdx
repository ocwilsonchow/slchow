...
Multi-AZ 处理 AZ failure。它不是 regional disaster recovery。这里的目标是 **RPO 不到一分钟** 与 **RTO 不到十五分钟**。那是可测目标，不是 AWS 产品保证。
