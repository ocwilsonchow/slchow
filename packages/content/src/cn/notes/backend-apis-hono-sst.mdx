...
当我建立 backend API 时，希望从 request 到 database 的路径很短，也希望 **validation**、**documentation** 与 **infrastructure** 都来自同一份代码。这套 stack 是 **Hono**、**Better Auth**、**Drizzle**、**PostgreSQL**、**Zod OpenAPI**、**Scalar**，以及跑在 **AWS Lambda** 或 **ECS Fargate** 上的 **SST**。
