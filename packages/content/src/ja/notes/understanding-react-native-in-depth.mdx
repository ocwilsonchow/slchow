...
Expo を [Next.js を深く理解する](/notes/understanding-next-js-in-depth) が Next.js を扱うように扱います：React core 周辺の policy と infrastructure。Next.js は browser 向けに URLs を React trees に map します。Expo は `app/` files と native modules を device 向け React trees に map します。
...
Fabric は renderer structures を immutable に保つことで thread-safe になるよう設計されています：updates は mutate せず clone します。私が実際に chase する almost every bug では二 thread が重要です：
