...
render を「React が component を呼んで DOM を更新する」と見なさなくなったとき、React は reasoning しやすくなります。その説明は、event handler の stale state、list items 間の state 移動、開発時 Effect の二重実行、transition の途中放棄など、多くの意外な挙動を説明する機構を飛ばします。
...
Host tree は renderer 依存です。`react-dom` は DOM nodes、React Native は native views、React Three Fiber は Three.js scene graph を作ります。React 自体は renderer 非依存——element と Fiber 層はすべての target で同じです。React Native が Expo 経由でそれをどう行うかは [React Native を深く理解する](/notes/understanding-react-native-in-depth) を参照。
